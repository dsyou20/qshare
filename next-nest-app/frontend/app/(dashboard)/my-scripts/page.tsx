...
export default function MyScriptsPage() {
  ...
  const filteredMyScripts = scripts.filter(script => 
    script.title.toLowerCase().includes(myScriptSearch.toLowerCase()) ||
    (script.description?.toLowerCase().includes(myScriptSearch.toLowerCase()) || false)
  );
  const filteredSharedScripts = sharedScripts.filter(script => 
    script.title.toLowerCase().includes(sharedScriptSearch.toLowerCase()) ||
    (script.description?.toLowerCase().includes(sharedScriptSearch.toLowerCase()) || false) ||
    (script.user?.email?.toLowerCase().includes(sharedScriptSearch.toLowerCase()) || false)
  );
  ...
  const handleShareScript = async () => {
    ...
    try {
      ...
      if (shareWithAll || selectedUsers.length > 0 || currentShares.length > 0) {
      ...
      setShareSuccess(shareWithAll || selectedUsers.length > 0 ? 
        '스크립트가 성공적으로 공유되었습니다.' : 
        '공유 설정이 변경되었습니다.');
  ...
  const handleCancelAllShares = async () => {
    if (!selectedScript || !confirm('모든 사용자와의 공유를 취소하시겠습니까?')) return;
  ...
  const handleQuickCancelShare = async (scriptId: string) => {
    ...
    try {
      ...
      if (script) {
        ...
        // 공유된 스크립트 목록에서 이 스크립트 제거
        const updatedSharedScripts = sharedScripts.filter(s => 
          s.id !== scriptId
        );
        setSharedScripts(updatedSharedScripts);
            ...
            <TextField
              ...
              InputProps={{
                ...
                endAdornment: myScriptSearch ? (
                  <IconButton size="small" onClick={() => setMyScriptSearch('')}>
                    <CloseIcon />
                  </IconButton>
                ) : null
              }}
                ...
                {filteredMyScripts.map((script) => (
                  <Card key={script.id}>
                    ...
                    <CardActions>
                      ...
                      <Tooltip title="공유">
                        <IconButton onClick={() => handleOpenShareModal(script)} color="primary">
                          <ShareIcon />
                        </IconButton>
                      </Tooltip>
                      {script.isPublic && (
            ...
            <TextField
              ...
              InputProps={{
                ...
                endAdornment: sharedScriptSearch ? (
                  <IconButton size="small" onClick={() => setSharedScriptSearch('')}>
                    <CloseIcon />
                  </IconButton>
                ) : null
              }}
              ...
              <Stack spacing={2}>
                {filteredSharedScripts.map((script) => (
                  <Card key={script.id}>
                    ...
                    <CardActions>
                      <Tooltip title="복제">
                        <IconButton onClick={() => handleCopyScript(script)} color="primary">
                          <ContentCopyIcon />
                        </IconButton>
                      </Tooltip>
                    </CardActions>
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  InputProps={{
                    ...params.InputProps,
                    endAdornment: searchLoading ? <CircularProgress color="inherit" size={20} /> : params.InputProps.endAdornment
                  }}
                />
              )}
              onChange={(_, value) => {
